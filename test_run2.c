		...
		if((*diag_msg).idiag_family==AF_INET)
		  {
		   ...
		   /*  fprintf(stdout,"DATA %x %u %x %x %x| %x %u - %u\n",(*diag_msg).id.idiag_src[0],ntohs((*diag_msg).id.idiag_sport),
			(*diag_msg).id.idiag_src[1],(*diag_msg).id.idiag_src[2],(*diag_msg).id.idiag_src[3],
			(*diag_msg).id.idiag_dst[0],ntohs((*diag_msg).id.idiag_dport),(*diag_msg).idiag_inode);
	*/
				...
				for(i=0;i<NLnumber;i++)
				 {
				   if(Nlinks[i].inode==Inode)
				    {
					fprintf(stdout,"  SOCKETS: (%u) %u(%u) %u(%u)\n",
					 Inode,Nlinks[i].l_ip,Nlinks[i].l_port,Nlinks[i].r_ip,Nlinks[i].r_port);
				    }
				 }
			  }
			 }
