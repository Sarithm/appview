...
int	get_pools(sock)
{
int	buf[2]={8,0xff030109};
int	len,i,id,hi,data[12],pp;
char	*p;
	...
	p=text;
	get_rec(sock,text,&len);
	if(len%26==0) pp=24; else if(len%50==0) pp=48; else pp=48;
	for(i=0;i<len/(pp+2);i++)
	  {
		id=*(p++); hi=*(p++);
		bcopy(p,data,pp); p+=pp;
		fprintf(stdout,"id:%2i, HI: %3i, MPAGES: %4i | %4i %10u %10u %10u %10u| %6u %10u %6u %10u| %4u %4u\n",
			id,hi,data[0],data[5],data[1],data[2],data[3],data[4],
...
/* - - - - - - - - - - - - - - - - - - - - - */
int	get_acc_list(sock)
{
int	buf[2]={8,0xff03010E};
int	i,len,ip;
unsigned	char	*p;

	if(send(sock,buf,8,0)==8)
	  {
		ioctl(sock,FIONBIO,&flag);
		p=text;
		get_rec(sock,text,&len);
...
/* - - - - - - - - - - - - - - - - - - - - - */
int	get_t_stats(int sock)
{
static	int	buf[2]={8,0xff03010B};
static	int	len;
unsigned int	ST[5];
	...
	if(write(sock,buf,8)==8)
	  {
	    ioctl(sock,FIONBIO,&flag);
	    while(get_rec(sock,text,&len)==2)
	     {
		bcopy(&text[22],(char *)ST,20);
		fprintf(stdout,"%-20s | %3i | %-7s | %6i %6i %6i %6i %6i\n",
		  text,text[20],s_types[text[21]],ST[0],ST[1],ST[2],ST[3],ST[4]);
	     }
	  }
	close(sock);
	exit(0);
...
{
static	int	buf[7]={8,0xff030108,0,0,0,0,0}; 
int	len,rc,i,j;
...
struct	tm *t_out;
	...
	if(strncmp(t_name,t_name_prev,22)!=0)
	  {
		...
		if(j==0&&strcmp(t_name,"TABLES")==0)
		  {
			if(strcmp(n_name[0],"pools")==0)	get_pools(sock);
			  else 
			if(strcmp(n_name[0],"access_list")==0)	get_acc_list(sock);
			  else
			if(strcmp(n_name[0],"stats")==0)	get_t_stats(sock);
		  }
		IT=get_tables(sock);
		close(sock);
	  }
	...
	if(j==3&&IT==1000) code=1; else		/* create table    */ 
	if(j==2&&IT<1000) code=23; else		/* update record   */
	if(j==2&&IT==1000) code=4; else		/* update table    */
	if(j==1&&IT<1000) code=24; else		/* delete record   */
	if(j==1&&IT==1000) code=2; else		/* delete table    */
	if(code==0)			err(20);
