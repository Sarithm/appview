...
int	TCK;
int	fd;
unsigned int	pid,ppid,Inode,PID;
...
struct	Nlink			*NL;
	...
	L=0;		/* listeners only, default */
	if(argc>1 && strcmp(argv[1],"-s")==0) L=1;
	 else if(argc>1 && strcmp(argv[1],"-a")==0) L=2;
	 else if(argc>2 && strcmp(argv[1],"-p")==0 && (PID=atol(argv[2]))>0) L=10;
	 else if(argc>1) { fprintf(stdout,"parameters: -s, -a, -p PID\n"); exit(4); }
			...
			if((*diag_msg).idiag_family==AF_INET || (*diag_msg).idiag_family==AF_INET6)
		  	 {
		    	  if(NLnumber<20000)
		      	   {
				NL=&Nlinks[NLnumber];
				(*NL).protocol=(*diag_msg).idiag_family;
				(*NL).l_ip=ntohl((*diag_msg).id.idiag_src[0]);
				(*NL).r_ip=ntohl((*diag_msg).id.idiag_dst[0]);
				(*NL).l_port=ntohs((*diag_msg).id.idiag_sport);
				(*NL).r_port=ntohs((*diag_msg).id.idiag_dport);
				(*NL).inode=(*diag_msg).idiag_inode;
				(*NL).nsinode=Nnet[ll].Inode;
				if((*NL).r_ip==0 && (*NL).r_port==0)
			  	 {
			    	   for(i=0;i<NListener;i++)
			     	   if(Listener[i].nsinode==(*NL).nsinode &&
				     Listener[i].protocol==(*NL).protocol &&
				     Listener[i].ls_ip==(*NL).l_ip && Listener[i].ls_port==(*NL).l_port) break;
			    	   if(i==NListener && NListener<10000)
			     	    {
					Listener[i].ls_ip=(*NL).l_ip;
					Listener[i].ls_port=(*NL).l_port;
					Listener[i].nsinode=(*NL).nsinode;
					Listener[i].protocol=(*NL).protocol;
					NListener++;
			     	    }
			 	  }
...
*/
				   ...
				   if(Nlinks[i].inode==Inode)
					...
					NCODE='L';
					 ...
					 if(L>0)
				      ...
				      Ok++;
				      if(L==0 && NCODE=='L' || L==1 && NCODE=='S' || L==2 || L==10 && PID==pid )
				        fprintf(stdout,"%u %s %u %u %u %u %c   %u %u %u %u\n",
				           pid,name,ppid,Nlinks[i].protocol,Nlinks[i].nsinode,Rtime,NCODE,
	...
	*/
	    ...
	    if(L==10 && Ok==1 && pid==PID) 	/* no network connections found */
	     {
